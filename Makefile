...
CPPFLAGS ?=-Wall -Wno-narrowing $(CUSTOM_CPPFLAGS)
CSTD ?=-std=c99
CPPSTD ?=-std=c++17
...
$(OBJ_DIR)%.o:%.cpp
	mkdir -p $(dir $@)
	$(CXX) $(CPPFLAGS) $(CPPSTD) $(BUILD_MODE_CFLAGS) $(DEP_INCLUDE_FLAGS) -c $< -o $@ $(CDEPFLAGS)

-include $(DEP_FILES)

$(DEP_LIBS_BUILD_DIR)depFile.dep: $(DEP_LIBS_DEPS)
	$(MAKE_CMD) -C $(DEPENDENCIES_DIR) PLATFORM=$(PLATFORM) BUILD_MODE=$(BUILD_MODE) RELEASE_OPTIM="$(RELEASE_OPTIM)" BUILD_DIR="$(DEP_LIBS_BUILD_DIR)" CUSTOM_CFLAGS="$(CUSTOM_CFLAGS)" CUSTOM_CPPFLAGS="$(CUSTOM_CPPFLAGS)" CSTD="$(CSTD)" CPPSTD="$(CPPSTD)"
